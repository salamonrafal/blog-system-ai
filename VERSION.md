...
- Zastąpiono systemowe okno wpisywania URL obrazka w toolbarze edytora treści artykułu własnym pickerem galerii mediów, który pozwala wyszukiwać i sortować obrazy, a po wyborze wstawia znacznik Markdown `![alt](...)` dokładnie w miejscu kursora.
- Wydzielono współdzielony moduł `media-image-picker`, dzięki czemu ten sam mechanizm obsługuje zarówno wybór `Grafiki nagłówkowej`, jak i nowy picker obrazów używany bezpośrednio w treści artykułu, wraz z pełną internacjonalizacją modala `PL/EN`.
- Poprawiono renderowanie treści artykułu na `blog_show`, rozszerzając parser markupu o obsługę obrazów ze ścieżkami lokalnymi zaczynającymi się od `/uploads/...`, dzięki czemu obrazy wstawione z galerii mediów są poprawnie widoczne na blogu, oraz dodano test regresyjny dla tego przypadku.
- Usprawniono domyślny `alt` dla obrazów wstawianych do treści artykułu, tak aby picker używał nazwy niestandardowej obrazka, a gdy jej brak, nazwy oryginalnej, z zachowaniem pierwszeństwa dla własnego tekstu zaznaczonego wcześniej przez użytkownika.
- Ustabilizowano układ pickera obrazów, rezerwując stałą wysokość odpowiadającą dwóm rzędom kafelków i dostosowując wysokość kart do desktopu, tabletu i mobile, aby filtrowanie wyników nie powodowało już skoków elementów interfejsu.
...
- Dodano read-only endpoint ostatnich powiadomień oraz rozbudowano backend powiadomień użytkownika o liczenie wszystkich wpisów, oznaczanie jako `przeczytane / nieprzeczytane`, usuwanie pojedynczego powiadomienia i czyszczenie całej listy, wykorzystując istniejące pole `displayedAt` jako źródło stanu odczytu.
