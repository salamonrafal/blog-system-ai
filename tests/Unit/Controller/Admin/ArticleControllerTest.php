...
final class ArticleControllerTest extends TestCase
{
    ...
    public function testIndexFiltersArticlesBySelectedCategory(): void
    {
        ...
        $articleRepository
            ->expects($this->once())
            ->method('findPaginatedForAdminIndex')
            ->with(1, 10, $selectedCategory, null, 'desc')
            ->willReturn([$article]);
    ...
    public function testIndexFiltersArticlesBySelectedStatus(): void
    {
        ...
        $articleRepository
            ->expects($this->once())
            ->method('countForAdminIndex')
            ->with(null, ArticleStatus::DRAFT)
            ->willReturn(1);
        $articleRepository
            ->expects($this->once())
            ->method('findPaginatedForAdminIndex')
            ->with(1, 10, null, ArticleStatus::DRAFT, 'desc')
            ->willReturn([$article]);
    ...
    public function testIndexKeepsCategoryAndStatusFiltersInPaginationState(): void
    {
        ...
        $articleRepository
            ->expects($this->once())
            ->method('countForAdminIndex')
            ->with($selectedCategory, ArticleStatus::REVIEW)
            ->willReturn(0);
        $articleRepository
            ->expects($this->once())
            ->method('findPaginatedForAdminIndex')
            ->with(1, 10, $selectedCategory, ArticleStatus::REVIEW, 'desc')
            ->willReturn([]);
    ...
    public function testIndexSortsArticlesByOldestUpdateDateWhenRequested(): void
    {
        ...
        $articleRepository
            ->expects($this->once())
            ->method('findPaginatedForAdminIndex')
            ->with(1, 10, $selectedCategory, ArticleStatus::REVIEW, 'asc')
            ->willReturn([]);
